...
function rdiv_frule_impl(X::AbstractVecOrMat, P::Permutation, dX::AbstractVecOrMat)
    Y = X / P
    dY = dX / P
    return Y, dY
end

function rdiv_frule_impl(X::AbstractVecOrMat, P::Permutation, dX::ZeroTangent)
    return X / P, ZeroTangent()
end

function ChainRulesCore.frule((_, dX, dP)::Tuple, ::typeof(/), X::AbstractVecOrMat, P::Permutation)
    return rdiv_frule_impl(X, P, dX)
end

# ===== rrule =====

function rdiv_rrule(X::AbstractVecOrMat, P::Permutation)
    Y = X / P
