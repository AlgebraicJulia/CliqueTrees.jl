function EnzymeRules.forward(
    ...
    y, dy = rdiv_frule_impl(x.val, A.val, x.dval, parent(A.dval))
...
function EnzymeRules.forward(
    ...
    y, dy = rdiv_frule_impl(x.val, A.val, ZeroTangent(), parent(A.dval))
    if needs_primal(config) && needs_shadow(config)
        return Duplicated(y, dy)
    elseif needs_shadow(config)
        return dy
...
function EnzymeRules.forward(
    ...
    y, dy = rdiv_frule_impl(x.val, A.val, x.dval, ZeroTangent())
    if needs_primal(config) && needs_shadow(config)
        return Duplicated(y, dy)
    elseif needs_shadow(config)
        return dy
...
function EnzymeRules.reverse(
    ...
    Δx, ΔL = rdiv_rrule_impl(xval, A.val, y, Δy)
