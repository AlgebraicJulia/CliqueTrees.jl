function EnzymeRules.forward(
    ...
    y, dy = rdiv_frule_impl(x.val, L.val, x.dval, L.dval)
...
function EnzymeRules.forward(
    ...
    y, dy = rdiv_frule_impl(x.val, L.val, ZeroTangent(), L.dval)
    if needs_primal(config) && needs_shadow(config)
        return Duplicated(y, dy)
    elseif needs_shadow(config)
        return dy
...
function EnzymeRules.forward(
    ...
    y, dy = rdiv_frule_impl(x.val, L.val, x.dval, ZeroTangent())
    if needs_primal(config) && needs_shadow(config)
        return Duplicated(y, dy)
    elseif needs_shadow(config)
        return dy
...
function EnzymeRules.reverse(
    ...
    Δx, ΔL = rdiv_rrule_impl(xval, L.val, y, Δy)
